...
def test_prepend():
    ...
    assert dll.head.next == None, "linked list emptry, so head should point to None"
    ...
    assert old_node.prev == new_node, "previous node of the old_node should be new_node"
    assert old_node.data == item, "checking the data of the old_node"
    assert str(dll) == "10,5", "string representation of dll should match 10,5"

def test_search():
    dll = DoubldyLinkedList()
    item = 5
    assert dll.search(item) == None, "item shouldnt be found in an empty list"
    dll.append(item)
    ...
    node = dll.search(20)
    assert node == None, "20 should no be found in the list"
